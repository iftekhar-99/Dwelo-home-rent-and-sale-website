...
const AdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const metricsResponse = await fetchWithAuth('/api/admin/dashboard');
      const metricsData = await metricsResponse.json();
      setMetrics(metricsData.data);

      const pendingPropertiesResponse = await fetchWithAuth('/api/admin/properties/pending');
      const pendingPropertiesData = await pendingPropertiesResponse.json();
      setPendingProperties(pendingPropertiesData.data.properties);

      const allPropertiesResponse = await fetchWithAuth('/api/admin/properties/all');
      const allPropertiesData = await allPropertiesResponse.json();
      setAllProperties(allPropertiesData.data.properties);
      
      const reportsResponse = await fetchWithAuth('/api/admin/reports/pending');
  ...
  const handlePropertyAction = async (propertyId, action, reason = '') => {
    try {
      await fetchWithAuth(`/api/admin/properties/${propertyId}/approve`, {
        method: 'PUT',
  ...
  const handleReportAction = async (reportId, action, details = '') => {
    try {
      await fetchWithAuth(`/api/admin/reports/${reportId}/handle`, {
        method: 'PUT',
